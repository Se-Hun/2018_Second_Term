...
void eval(char *cmdline) 
{
	...
	sigemptyset(&mask);
	sigaddset(&mask, SIGCHLD); //자식 프로세스가 종료되었을때 발생
	sigaddset(&mask, SIGINT); //ctrl+c가 눌러졌을때 발생
	sigaddset(&mask, SIGTSTP); //ctrl+z가 눌러졌을때 발생
...
void sigchld_handler(int sig) 
{
	...
	while((pid = waitpid(fgpid(jobs), &child_status, WUNTRACED|WNOHANG)) > 0) {
